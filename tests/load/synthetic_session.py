...
session driver constructs a :class:`Moneta` handle inside a context
manager; phase functions take the handle explicitly. The virtual-clock
monkey-patch on ``moneta.api.time`` is unchanged — the handle's methods
still call ``time.time()`` against the api module's ``time`` import.
...
(or a literal int), never ``hash()``. See ``docs/testing.md``
(Documentarian follow-up from Pass 4) for the externalized version of
this rule.
...
for the duration of ``run_session_internal``. All internal
``time.time()`` calls in the api layer see the virtual clock.
``Consolidation``'s ``run_pass`` accepts ``now`` as an explicit
parameter, so the reducer + decay points also see virtual time via
``Moneta.run_sleep_pass``'s ``now = time.time()``.
"""
...
def _embed(topic: str, variant: int = 0) -> List[float]:
    ...
    ``hash()`` is randomized per-process by default (PYTHONHASHSEED) —
    any use of ``hash()`` for seeding would make the session
    non-reproducible across runs. This subtle bug surfaced during Pass
    4 harness development and is worth noting: ANY test that seeds
    randomness from string hashes must use hashlib (or a fixed-seed
    substitute), never ``hash()``.
    """
...
def _phase_research_burst(
    m: Moneta, clock: VirtualClock, metrics: SessionMetrics
) -> List[UUID]:
    ...
    while elapsed < duration_seconds:
        ...
        if rng.random() < (1.0 / 6.0):
            ...
            if results:
                ...
                m.signal_attention(weights)
                metrics.total_attention_signals += len(weights)

        clock.advance(1.0)
        elapsed += 1.0
        metrics.max_ecs_size = max(metrics.max_ecs_size, m.ecs.n)
...
def _phase_context_switch(
    ...
    for _ in range(15):
        topic = rng.choice(TOPICS)
        m.query(_embed(topic, variant=rng.randrange(1000)), limit=5)
        metrics.total_queries += 1
        clock.advance(2.0)
...
def _phase_recall(
    ...
    for _ in range(20):
        results = m.query(_embed(target_topic, variant=0), limit=3)
        metrics.total_queries += 1
        if results:
            m.signal_attention({results[0].entity_id: 0.1})
...
def run_session_internal(clock: VirtualClock) -> SessionMetrics:
    ...
    restoration in a CLI entry point). The handle is constructed and
    cleaned up via context manager.
    """
    random.seed(1337)
    metrics = SessionMetrics()
    ...
    with Moneta(config) as m:
        ...
        research_ids = _phase_research_burst(m, clock, metrics)
        known.extend(research_ids)

        # Sleep pass 1 — immediately after burst
        _take_pass_snapshot(m, clock, metrics, session_start)

        # Phase 2 — conversation (30s -> ~14m30s)
        _phase_conversation(
            m, clock, metrics, known, duration_seconds=14 * 60
        )
        ...
        clock.advance(5 * 60)
        _phase_context_switch(m, clock, metrics)

        # Phase 4 — recall (reinforce one topic)
        _phase_recall(m, clock, metrics, target_topic=TOPICS[0])

        # Sleep pass 3 — late session
        _take_pass_snapshot(m, clock, metrics, session_start)

        # Final distribution capture
        metrics.final_ecs_n = m.ecs.n
        attended_hist: dict[int, int] = {}
        util_hist: dict[float, int] = {}
        for memory in m.ecs.iter_rows():
...
def format_report(metrics: SessionMetrics) -> str:
    ...
    for k in sorted(metrics.final_attended_count_distribution.keys()):
        lines.append(
            f"  attended={k}: "
            f"{metrics.final_attended_count_distribution[k]} entities"
        )
    lines.append("")
    ...
    for k in sorted(metrics.final_utility_distribution.keys()):
        lines.append(
            f"  u={k:4.2f}: "
            f"{metrics.final_utility_distribution[k]} entities"
        )
    lines.append("=" * 64)
    return "\n".join(lines)
...
def _assert_tier1(metrics: SessionMetrics) -> None:
    ...
    assert (
        metrics.final_ecs_n
        == metrics.total_deposits - metrics.total_pruned
    ), (
        f"final_ecs_n={metrics.final_ecs_n} "
        f"!= deposits ({metrics.total_deposits}) "
        f"- pruned ({metrics.total_pruned})"
    )

    # Pass 1: fresh burst, no effects.
    p1 = metrics.pass_snapshots[0]
    assert p1.attention_updated == 0, (
        f"pass 1 attention updates: expected 0, "
        f"got {p1.attention_updated}"
    )
    assert p1.pruned == 0
    assert p1.staged == 0
    ...
    # Pass 2: conversation phase keeps entities fresh via query-path
    # decay + attention boosts; no prunes, no stages.
    ...
    # All pass manifests empty — commit transitions
    # STAGED -> CONSOLIDATED in-pass, so manifest drains at every
    # boundary.
    manifest_sizes = [s.manifest_size for s in metrics.pass_snapshots]
    assert all(size == 0 for size in manifest_sizes), (
        f"manifest should drain after commit at every pass, "
        f"got {manifest_sizes}"
    )

    # Both consolidation paths fire at least once across the session.
    assert metrics.total_pruned >= 1, (
        "session should exercise prune path"
    )
    assert metrics.total_staged >= 1, (
        "session should exercise stage path"
    )
...
def _assert_tier2(metrics: SessionMetrics) -> None:
    ...
    original proposal:
      ...
      - LOOSENED: pass_snapshots[1].attention_updated from [88,100] to
        [85,105].
    """
    # Exact canonical totals — deterministic under fixed seed +
    # hashlib embedder.
    ...
    assert 85 <= p2.attention_updated <= 105, (
        f"pass 2 attention_updated outside [85,105]: "
        f"{p2.attention_updated}"
    )
    ...
    # Recall phase reinforces a single entity up to utility = 1.0
    # exactly.
    assert max(metrics.final_attended_count_distribution.keys()) >= 10, (
        "recall phase should produce at least one heavily attended "
        "entity"
    )
...
def test_phase1_synthetic_session_completion_gate(monkeypatch) -> None:
    ...
    ``test_phase1_synthetic_session_completion_gate_load`` adds
    Tier 2 canonical-range assertions.
    """
    clock = VirtualClock()
    monkeypatch.setattr(
        moneta_api, "time", SimpleNamespace(time=clock.time)
...
def test_phase1_synthetic_session_completion_gate_load(monkeypatch) -> None:
    ...
    is selectable via ``pytest -m load``. Contains all Tier 1
    assertions plus the Tier 2 deterministic-value and range
    assertions approved in Pass 4.
    """
    clock = VirtualClock()
    monkeypatch.setattr(
        moneta_api, "time", SimpleNamespace(time=clock.time)
