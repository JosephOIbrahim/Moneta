...
deposit -> query -> signal_attention -> run_sleep_pass -> manifest
flow through UsdTarget instead of MockUsdTarget.

After the singleton surgery (DEEP_THINK_BRIEF_substrate_handle.md), the
fixtures yield :class:`Moneta` handles with ``use_real_usd=True``;
substrate state is read off the handle (``m.authoring_target``,
``m.ecs``) rather than off a module-level singleton.

These tests require pxr (OpenUSD Python bindings). Under plain Python
they are skipped entirely via ``pytest.importorskip``. Run under
hython::
...
from moneta import Moneta, MonetaConfig  # noqa: E402
...
@pytest.fixture
def fresh_moneta_usd() -> Iterator[Moneta]:
    """Yield a Moneta handle with ``use_real_usd=True``, in-memory layers."""
    ...
    with Moneta(config) as substrate:
        yield substrate


@pytest.fixture
def fresh_moneta_mock() -> Iterator[Moneta]:
    """Yield a Moneta handle with MockUsdTarget (Phase 1 default)."""
    config = MonetaConfig.ephemeral(
        use_real_usd=False,
...
def _deposit_n(
    m: Moneta, n: int, *, protected_floor: float = 0.0
) -> list[UUID]:
    """Deposit n memories with distinct embeddings on the given handle."""
    ids = []
    for i in range(n):
        eid = m.deposit(
            payload=f"memory #{i}",
...
def _force_staging(m: Moneta, eids: list[UUID]) -> None:
    """Manipulate the handle's ECS to make entities match staging criteria.
    ...
    for eid in eids:
        ...
        if idx is not None:
            ...
            m.ecs._last_evaluated[idx] = time.time()
...
class TestRealUsdEndToEnd:
    def test_deposit_query_attention_sleep_under_real_usd(
        ...
        """
        m = fresh_moneta_usd
        assert isinstance(m.authoring_target, UsdTarget)

        # Deposit
        eid = m.deposit("end-to-end test", [1.0, 0.0, 0.0])
        ...
        target = m.authoring_target
        prim_path = f"/Memory_{eid.hex}"
        prim = target.stage.GetPrimAtPath(prim_path)
        assert prim.IsValid(), (
            f"prim at {prim_path} should exist after staging"
        )
...
class TestProtectedRouting:
    def test_protected_memory_routes_to_protected_sublayer(
        ...
        eid = m.deposit(
            "core identity", [1.0, 0.0, 0.0], protected_floor=0.1
        )
        ...
        assert prim_spec is not None, (
            "protected memory should be on protected layer"
        )
...
class TestBatchCap:
    def test_large_staging_respects_500_prim_batch_cap(
        ...
        total_prims = sum(
            target.get_prim_count(name) for name in rolling_names
        )
        assert total_prims == n, (
            f"expected {n} prims total, got {total_prims}"
        )

        # Verify a sample prim from the first and last batch
        first_prim = target.stage.GetPrimAtPath(
            f"/Memory_{eids[0].hex}"
        )
        last_prim = target.stage.GetPrimAtPath(
            f"/Memory_{eids[-1].hex}"
        )
...
class TestABEquivalence:
    def test_mock_vs_real_produce_equivalent_state_transitions(
        self, fresh_moneta_mock: Moneta
    ) -> None:
        """Commandment 7: adversarial verification.

        Verify that ``MockUsdTarget`` and ``UsdTarget`` produce
        equivalent ``ConsolidationResult`` (same pruned/staged counts)
        for identical inputs. The selection logic is in
        ``consolidation.py`` — both targets should produce the same
        state transitions.
        """
        # --- Run under mock ---
        m_mock = fresh_moneta_mock
        n_stage = 5
        n_prune = 3
        _ = 2  # n_keep entities are not selected by either pass

        stage_ids = _deposit_n(m_mock, n_stage)
        prune_ids = _deposit_n(m_mock, n_prune)
        _deposit_n(m_mock, 2)

        # Force staging criteria on stage_ids
        _force_staging(m_mock, stage_ids)

        # Force pruning criteria on prune_ids: utility < 0.1, attended < 3
        for eid in prune_ids:
            idx = m_mock.ecs._id_to_row.get(eid)
            if idx is not None:
                m_mock.ecs._utility[idx] = 0.05
                m_mock.ecs._attended[idx] = 1
                m_mock.ecs._last_evaluated[idx] = time.time()

        mock_result = m_mock.run_sleep_pass()

        # --- Run under real USD on a fresh handle ---
        usd_config = MonetaConfig.ephemeral(
            use_real_usd=True,
            usd_target_path=None,
            half_life_seconds=60.0,
        )
        with Moneta(usd_config) as m_real:
            stage_ids2 = _deposit_n(m_real, n_stage)
            prune_ids2 = _deposit_n(m_real, n_prune)
            _deposit_n(m_real, 2)

            _force_staging(m_real, stage_ids2)

            for eid in prune_ids2:
                idx = m_real.ecs._id_to_row.get(eid)
                if idx is not None:
                    m_real.ecs._utility[idx] = 0.05
                    m_real.ecs._attended[idx] = 1
                    m_real.ecs._last_evaluated[idx] = time.time()

            real_result = m_real.run_sleep_pass()

        # --- Compare ---
        assert mock_result.pruned == real_result.pruned, (
            f"pruned mismatch: mock={mock_result.pruned}, "
            f"real={real_result.pruned}"
        )
        assert mock_result.staged == real_result.staged, (
            f"staged mismatch: mock={mock_result.staged}, "
            f"real={real_result.staged}"
        )
...
class TestSublayerRotationDuringSleepPass:
    def test_rotation_across_passes_preserves_all_prims(
        ...
        ``author_stage_batch`` call go to the current sublayer, and
        rotation triggers on the next call when the accumulated count
        exceeds the cap. This test exercises rotation by running two
        sleep passes: the first fills the sublayer past the cap, the
        second rotates.
        """
        m = fresh_moneta_usd
        target = m.authoring_target
        ...
        # Pass 1: deposit and consolidate 8 prims -> fills past cap=5
        eids_1 = _deposit_n(m, 8)
        _force_staging(m, eids_1)
        r1 = m.run_sleep_pass()
        assert r1.staged == 8

        rolling_after_1 = [
            nm for nm in target.sublayer_names
            if nm != PROTECTED_SUBLAYER_NAME
        ]
        assert len(rolling_after_1) == 1, "first pass fills one sublayer"
        assert target.get_prim_count(rolling_after_1[0]) == 8

        # Pass 2: deposit and consolidate 4 more
        # -> rotation triggers (8 >= cap=5)
        eids_2 = _deposit_n(m, 4)
        _force_staging(m, eids_2)
        r2 = m.run_sleep_pass()
        assert r2.staged == 4
        ...
        assert len(rolling_after_2) == 2, (
            f"second pass should trigger rotation, got "
            f"{len(rolling_after_2)} sublayers"
        )

        # Verify no prims lost
        total = sum(
            target.get_prim_count(nm) for nm in rolling_after_2
        )
        ...
        for eid in eids_1 + eids_2:
            ...
            assert prim.IsValid(), (
                f"prim for {eid} should survive rotation"
            )
...
class TestPostSleepPassReaderConsistency:
    def test_prims_visible_to_traverse_after_sleep_pass(
        ...
        """After a sleep pass consolidates entities, a fresh ``Traverse``
        sees the authored prims. Validates the narrow-lock contract at
        the integration layer: ``ChangeBlock`` makes prims visible
        in-memory, ``flush()`` makes them durable.
        """
        m = fresh_moneta_usd
        target = m.authoring_target
        ...
        assert len(authored_pre) == 0, (
            "no Memory prims in USD before sleep pass"
        )
        ...
        assert len(authored_post) == n, (
            f"expected {n} Memory prims after sleep pass, "
            f"got {len(authored_post)}"
        )
